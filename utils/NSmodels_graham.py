...
def CARMAsine_gen_prior_dynamic_code(p, q):
    ...
    dynamic_code_str =  dynamic_code_str + indent + "A1 = yield jaxns.Prior( tfpd.Uniform(0,0.08), name = 'A1') "+ "\n"
    #dynamic_code_str =  dynamic_code_str + indent + "phi = yield jaxns.Prior( tfpd.Normal(-np.pi,np.pi), name = 'phi') "+ "\n"
    dynamic_code_str =  dynamic_code_str + indent + "A2 = yield jaxns.Prior( tfpd.Normal(0,0.08), name = 'A2')"+ "\n"
    dynamic_code_str =  dynamic_code_str + indent + "t0 = yield jaxns.Prior( tfpd.Uniform(0, 6.67), name ='t0')"+ "\n\n"
...
def OBPL_generative_prior(time  = None):
    ...
    scale = yield jaxns.Prior( tfpd.LogNormal(-0.6, 1.15), name = 'scales') # 1/years
    amp = yield jaxns.Prior( tfpd.LogNormal(-4, 1.15), name = 'amp')
    err_scale = yield jaxns.Prior(tfpd.Uniform(0.0,1.5), name='err_scale')
    
    return alpha_l, alpha_h, scale, amp, err_scale

#One bend power law likelihood
def OBPL_log_likelihood_model(alpha_l, alpha_h, scale, amp, err_scale,  n_component=10, time = None, y = None, y_errs = None ):
...
def OBPLsine_generative_prior(time  = None):
    ...
    return alpha_l, alpha_h, scale, amp, A1, A2, t0, err_scale


#One bend power law + sine likelihood
def OBPLsine_log_likelihood_model(alpha_l, alpha_h, scale, amp, A1, A2, t0, err_scale, n_component=10, time = None, y = None, y_errs = None ):
