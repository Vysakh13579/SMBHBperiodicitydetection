...
def DRW_generative_prior(time =None):
    log_amp = yield jaxns.Prior( tfpd.Uniform(-5, 5), name = 'log_amp')
...
def DRW_sine_generative_prior(time=None):
    ...
    t0 = yield jaxns.Prior( tfpd.Uniform(0, 10), name ='t0')
    #nu = yield jaxns.Prior( tfpd.Uniform(0.0, 1.), name = 'nu')
...
def DRW_sine_log_likelihood_model(log_amp, log_bend_freq, A1, t0, phi, err_scale, time = None, y = None, y_errs = None):
...
def CARMAsine_gen_prior_dynamic_code(p, q):
    ...
    dynamic_code_str =  dynamic_code_str + indent + "t0 = yield jaxns.Prior( tfpd.Uniform(0, 10), name ='t0')"+ "\n\n"
...
def CARMAsine__log_likelihood_dynamic_code(p, q):
    ...
    dynamic_code_str = dynamic_code_str + "log_sigma, A1, phi, t0, err_scale, time = None, y = None, y_errs = None):" + "\n\n" 
    dynamic_code_str = dynamic_code_str + indent + "kernel = kernels.quasisep.CARMA.init("
...
def OBPLsine_log_likelihood_model(alpha_l, alpha_h, log_bend_freq, log_norm, A1, phi, t0, err_scale, n_component=10, time = None, y = None, y_errs = None ):
