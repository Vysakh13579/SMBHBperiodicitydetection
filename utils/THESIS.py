    ...
    def run_NSsampler(self,name=None, num_par_samplers=1):
        ...
        if name==None:
            ...
            for key, value in self.nsmodels.items():
                ...
                self.nssamplers[key] = DefaultNestedSampler(model = value, max_samples = 1e6, num_live_points = 400,  num_parallel_workers= num_par_samplers)
            ...
            self.nssamplers[name] = DefaultNestedSampler(model = self.nsmodels[name], max_samples = 1e6,  num_live_points = 400,num_parallel_workers= num_par_samplers)
...
class LightCurveSampler:
    def __init__(self, N =2**10, mean = 1.0, dt=10.0, rms=1.0, simulatorSEED = 1079, verbose = True):
        ...
        mean        :     mean value of lightcurve: default value = 1.0
        ...
        if isinstance(mean, float):
            ...
            self.partialsignal = -1.0
        else:
            self.mean = 1.0
            self.partialsignal = mean
    ...
    def LCsimulator(self):
        ...
        # elif len(self.signal) == self.N:
        #     self.lc = self.sim.simulate(spectrum)
        #     self.lc.counts = self.lc.counts + self.signal
        # else : 
        #     print(" WARNING!!! : input signal length does not match N")
    ...
    def sampler(self, OBSpNight = 3, NightsperWINDOW = 7, NumofWINDOW = 9 , OBSperiod = 8, WINDOWwidth = 10, dataLOSSfrac = 0, noiseSIGMA = 0, samplerSEED = 100, plot=True, returnVAL =True ):
        ...
        if not isinstance(self.partialsignal, float):
            simFLUX = simFLUX +  self.partialsignal(simTIME)
        ...
        if plot:
            ...
            if not isinstance(self.partialsignal, float):
                plt.plot(self.lc.time/365,  self.lc.counts + self.partialsignal(self.lc.time/365),'k', alpha=0.5)    
                #plt.plot(self.lc.time/365, self.partialsignal(self.lc.time/365) + self.mean, 'r', alpha=0.5, label = 'mean signal')
            else:
                plt.plot(self.lc.time/365,  self.lc.counts, 'k', alpha=0.5)
                plt.plot(self.lc.time/365, self.mean*np.ones_like(self.lc.time), 'r', alpha=0.5, label = 'mean signal')
