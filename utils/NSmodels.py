...
def DRW_log_likelihood_model(log_amp, log_bend_freq, time = None, y = None, y_errs = None ):
    
    kernel = jnp.exp(log_amp) * kernels.Exp(scale = 1/jnp.power(10,log_bend_freq))
    
    gp = GaussianProcess(kernel, time, diag=(y_errs)**2, mean=0)
    
    return gp.log_probability(y)


# DRW + linear model log likelihood model
def DRW_linear_log_likelihood_model(log_amp, log_bend_freq, m, b, time = None, y = None, y_errs = None ):
    
    kernel = jnp.exp(log_amp) * kernels.Exp(scale = 1/jnp.power(10,log_bend_freq))
    mean_params = {'m': m, 'b':b}
    
    gp = GaussianProcess(kernel, time, diag=(y_errs)**2, mean=partial(linear_model, mean_params))
...
def DRW_sine_generative_prior(time=None):
    ...
    A1 = yield jaxns.Prior( tfpd.Uniform(0,10), name = 'A1')
    phi = yield jaxns.Prior( tfpd.Uniform(-np.pi,np.pi), name = 'phi')
    t0 = yield jaxns.Prior( tfpd.Uniform(0, 5), name ='t0')
    #nu = yield jaxns.Prior( tfpd.Uniform(0.0, 1.), name = 'nu')
    
    return log_amp, log_bend_freq,  A1, t0, phi#err_scale, A1, A2, t0 #nu
...
def sine_curve_model(params, time):
    ...
    return params['A1']* jnp.sin(2*jnp.pi*time/params['t0'] + params['phi'])# + params['A2']* jnp.sin(2*jnp.pi*time/params['t0']) 
...
def DRW_sine_linear_log_likelihood_model(log_amp, log_bend_freq, A1, A2, t0, m, b, time = None, y = None, y_errs = None):
    
    kernel = jnp.exp(log_amp) * kernels.Exp(scale = 1/jnp.power(10,log_bend_freq))
    
    mean_params = {'A1': A1, 'A2':A2, 't0':t0, 'm':m, 'b':b}
    
    gp = GaussianProcess(kernel, time, diag=(y_errs)**2, mean= partial(sine_linear_model, mean_params) )
    
    return gp.log_probability(y)

# DRW + sine  model loglikelihood 
def DRW_sine_log_likelihood_model(log_amp, log_bend_freq, A1, t0, phi, time = None, y = None, y_errs = None):
    
    kernel = jnp.exp(log_amp) * kernels.Exp(scale = 1/jnp.power(10,log_bend_freq))
    
    mean_params = {'A1': A1, 'phi':phi, 't0':t0}
    
    gp = GaussianProcess(kernel, time, diag=(y_errs)**2, mean= partial(sine_curve_model, mean_params) )
...
def CARMA__log_likelihood_dynamic_code(p, q):
    ...
    dynamic_code_str = dynamic_code_str + "log_sigma, time = None, y = None, y_errs = None):" + "\n\n" 
    ...
    dynamic_code_str = dynamic_code_str + indent + "gp = GaussianProcess(kernel, time, diag=(y_errs)**2, mean=0)" + "\n\n"
...
def CARMAsine_gen_prior_dynamic_code(p, q):
    ...
    dynamic_code_str =  dynamic_code_str + indent + "A1 = yield jaxns.Prior( tfpd.Uniform(0,10), name = 'A1') "+ "\n"
    ...
    dynamic_code_str = dynamic_code_str + "log_sigma, A1, phi, t0"#, err_scale" 
...
def CARMAsine__log_likelihood_dynamic_code(p, q):
    ...
    dynamic_code_str = dynamic_code_str + "log_sigma, A1, phi, t0, time = None, y = None, y_errs = None):" + "\n\n" 
    dynamic_code_str = dynamic_code_str + indent + "kernel = kernels.quasisep.CARMA.init("
    ...
    dynamic_code_str = dynamic_code_str + indent + "mean_params = {'A1': A1, 'phi':phi, 't0':t0}"+  "\n\n"
    
    dynamic_code_str = dynamic_code_str + indent + "gp = GaussianProcess(kernel, time, diag=(y_errs)**2, mean=partial(sine_curve_model, mean_params))" + "\n\n"
...
def OBPL_log_likelihood_model(alpha_l, alpha_h, log_bend_freq, log_norm,  n_component=10, time = None, y = None, y_errs = None ):
    
    psd = OneBendPowerLaw([jnp.power(10, log_norm), alpha_l, jnp.power(10, log_bend_freq), alpha_h], free_parameters=[True]*4)
...
def OBPLsine_generative_prior(time  = None):
    ...
    return alpha_l, alpha_h, log_bend_freq, log_norm, A1, phi, t0#, err_scale


#One bend power law + sine likelihood
def OBPLsine_log_likelihood_model(alpha_l, alpha_h, log_bend_freq, log_norm, A1, phi, t0, n_component=4, time = None, y = None, y_errs = None ):
